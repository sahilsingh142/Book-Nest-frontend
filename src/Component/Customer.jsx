  ...
  return (
    <div className="relative bg-zinc-50">
      ...
      <nav className="sticky top-0 z-20 border-b border-zinc-200/80 bg-zinc-50/70 backdrop-blur-md">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 sm:gap-4 px-4 py-3 sm:px-10 sm:py-4">
          <h1 className="group flex shrink-0 cursor-pointer text-lg sm:text-xl font-black tracking-tight">
          ...
          <div className="relative min-w-0 flex-1 sm:max-w-md">
            <FiSearch className="pointer-events-none absolute left-3.5 sm:left-4 top-1/2 -translate-y-1/2 text-zinc-400" />
            <input
              className="w-full rounded-full border border-zinc-300 bg-white py-2 sm:py-2.5 pl-10 sm:pl-11 pr-9 sm:pr-10 text-sm font-medium text-zinc-700 outline-none transition-colors focus:border-emerald-500"
              type="text"
            ...
            {search && (
              <button
                onClick={() => setSearch("")}
                className="absolute right-3 sm:right-3.5 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-600"
      ...
      <section className="relative mx-auto max-w-7xl px-4 sm:px-10">
        ...
        <div className="flex flex-wrap gap-2 pt-6 sm:pt-10">
          ...
          <button onClick={() => setActiveCategory("All")}
            className={`rounded-full px-3.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold transition-all duration-300 ${activeCategory === "All"
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`rounded-full px-3.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold transition-all duration-300 ${activeCategory === cat
          ...
          <div className="grid grid-cols-1 gap-5 sm:gap-8">
            {filteredBusiness.map((business, i) => (
              ...
              <div key={business._id}
                className="bn-rise bn-ticket group relative cursor-pointer overflow-hidden rounded-2xl bg-white p-5 sm:p-6 shadow-sm ring-1 ring-zinc-200 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-zinc-200"
              >
                <span className="absolute right-4 top-4 rounded-full bg-zinc-900 px-2.5 py-1 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-white">
                  {business.category}
                </span>

                <div className="flex items-center gap-3">
                  <div className="flex h-11 w-11 sm:h-12 sm:w-12 shrink-0 tracking-wider items-center justify-center rounded-xl bg-emerald-50 text-lg sm:text-xl font-black text-emerald-600">
                    {initialsOf(business.businessName)}
                  </div>
                  <h3 className="pr-16 text-lg sm:text-xl font-extrabold leading-tight tracking-tight text-zinc-600">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center justify-between gap-3 sm:flex-col sm:items-end sm:justify-center sm:gap-2">
                    <button
                      ...
                      className="px-4 py-1.5 sm:px-6 sm:py-2 text-xs sm:text-sm font-medium rounded-full bg-mauve-600 text-zinc-200 hover:scale-95 duration-200 hover:bg-mauve-700 cursor-pointer"
                    >
...

                    {
                      business.status === "busy" ? (
                        ...
                        <span className="font-bold tracking-widest text-sm text-emerald-500">
                          Available
                        </span>
                      )
                    }
